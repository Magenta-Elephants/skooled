...
router.get('/classDetail', ensureAuthorized, (req, res) => {
  ...
  // if (allAssignmentsGrades.length > 0 && classAssignments.length > 0) {
  //   var specificStudentGrades = [];
  //   for (var i = 0; i < classAssignments.length; i++) {
  //     retrieveStudentAssignmentGradeAsync(classAssignments[i].id_assignment, studentId)
  //       .then(grade => {
  //         console.log('\nGRADE 2: ', grade);
  //         specificStudentGrades.push(grade.models[0].attributes);

  //         if (specificStudentGrades.length === classAssignments.length) {
  //           return [specificStudentGrades, classAssignments];
  //         }
  //       })
  //       .then(assignmentsAndGrades => {
  //         console.log('\nASSIGNMENTS AND GRADES: ', assignmentsAndGrades);
  //         res.json(assignmentsAndGrades);
  //       })
  //       .catch(error => {
  //         console.log('ERROR WITH RETRIEVE STUDENT ASSIGNMENT GRADE: ', error);
  //       })
  //   }
  // }
